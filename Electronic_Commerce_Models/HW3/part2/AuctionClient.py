...
class AuctionClient:
    def __init__(self, value, clients_num, insurances_num):
        ...
        self.auctions_left = insurances_num
        self.won = False

    def decide_bid(self, t, duration):

        if self.clients_num < self.insurances_num:
            if duration > self.value + 0.1:
                return 0
            return -1

        remaining_ratio = (self.clients_num - t) / self.clients_num
        bid = self.value * (1 - np.exp(-remaining_ratio))

        # Adding a small margin to prevent underbidding
        margin = 0.11
        return bid + margin
