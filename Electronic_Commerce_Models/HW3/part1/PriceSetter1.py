...
class PriceSetter1:
    ...
    def update(self, t, outcome):
        ...
        """
        if outcome:  # Product was sold
            self.lower_bound = self.current_price
        else:  # Product was not sold
            self.upper_bound = self.current_price

        self.current_price = (self.lower_bound + self.upper_bound) / 2

        # Check if we've converged
        if self.upper_bound - self.lower_bound < self.epsilon:
            # Once converged, set price slightly below the threshold
            self.current_price = self.lower_bound - self.epsilon
